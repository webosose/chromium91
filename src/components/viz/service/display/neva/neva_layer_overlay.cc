...
#if defined(USE_NEVA_MEDIA)
  video_window_geometry_manager_ =
      ui::OzonePlatform::GetInstance()->GetVideoWindowGeometryManager();
#endif  // defined(USE_NEVA_MEDIA)
}
...
void NevaLayerOverlayProcessor::Process(
  ...
  if (video_window_geometry_manager_ &&
    ...
    video_window_geometry_manager_->BeginOverlayProcessor(surface_handle_);
#endif  // defined(USE_NEVA_MEDIA)
  ...
  if (video_window_geometry_manager_ &&
      video_window_geometry_manager_->IsInitialized())
    video_window_geometry_manager_->EndOverlayProcessor(surface_handle_);
#endif  // defined(USE_NEVA_MEDIA)
}
  ...
  for (auto it = quad_list->begin(); it != quad_list->end(); it = next_it) {
    ...
    if ((*it)->material == DrawQuad::Material::kVideoHole) {
      ...
      render_pass->ReplaceExistingQuadWithSolidColor(
          it, SK_ColorTRANSPARENT, SkBlendMode::kSrcOver);
      if (video_window_geometry_manager_ &&
          video_window_geometry_manager_->IsInitialized()) {
