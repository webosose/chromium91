...
MOCKPORT=${MOCKPORT:-8888}
WEBPORT=${WEBPORT:-8887}
...
CREATE_EXPECTATIONS()
{
    for FILENAME in `find * -type f` __IP__ __PORT__ ""
    do
        with_backoff curl -sS --request PUT $expect_path $params --data \
"`cat <<EOF
{
...
java $logoff_flag $logoff_all_flag -jar mockserver-netty-5.11.1-jar-with-dependencies.jar -serverPort $MOCKPORT &
MOCKSRVPID=$!
...
./tests_server.bash $WEBPORT $CUR_DIR/../tests &
WEBSRVPID=$!
