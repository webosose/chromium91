...
MojoMediaPlayerService::MojoMediaPlayerService(
  ...
  media_player_neva_.reset(media::MediaPlayerNevaFactory::CreateMediaPlayerNeva(
      this, media_player_type, base::ThreadTaskRunnerHandle::Get(), app_id));
}

MojoMediaPlayerService::~MojoMediaPlayerService() {}

void MojoMediaPlayerService::Connect(ConnectCallback callback) {
  mojo::PendingAssociatedRemote<neva_media::mojom::MediaPlayerListener>
      pending_remote;
  std::move(callback).Run(pending_remote.InitWithNewEndpointAndPassReceiver());
  remote_client_.Bind(std::move(pending_remote));
}
...
void MojoMediaPlayerService::OnMediaMetadataChanged(
    base::TimeDelta duration,
    const gfx::Size& coded_size,
    const gfx::Size& natural_size,
    bool success) {
  remote_client_->OnMediaMetadataChanged(duration, coded_size, natural_size,
                                         success);
}

void MojoMediaPlayerService::OnLoadComplete() {
  remote_client_->OnLoadComplete();
}

void MojoMediaPlayerService::OnPlaybackComplete() {
  remote_client_->OnPlaybackComplete();
}

void MojoMediaPlayerService::OnSeekComplete(
    const base::TimeDelta& current_time) {
  remote_client_->OnSeekComplete(current_time);
}

void MojoMediaPlayerService::OnMediaError(int error) {
  remote_client_->OnMediaError(error);
}

void MojoMediaPlayerService::OnVideoSizeChanged(const gfx::Size& coded_size,
                                                const gfx::Size& natural_size) {
  remote_client_->OnVideoSizeChanged(coded_size, natural_size);
}

void MojoMediaPlayerService::OnMediaPlayerPlay() {
  remote_client_->OnMediaPlayerPlay();
}

void MojoMediaPlayerService::OnMediaPlayerPause() {
  remote_client_->OnMediaPlayerPause();
}
...
void MojoMediaPlayerService::OnAudioTracksUpdated(
  ...
  remote_client_->OnAudioTracksUpdated(audio_track_info);
}

void MojoMediaPlayerService::OnTimeUpdate(base::TimeDelta current_timestamp,
                                          base::TimeTicks current_time_ticks) {
  remote_client_->OnTimeUpdate(current_timestamp, base::TimeTicks::Now());
}
