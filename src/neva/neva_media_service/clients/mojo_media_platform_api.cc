...
scoped_refptr<media::MediaPlatformAPI> MojoMediaPlatformAPI::Create(
    ...
    const media::MediaPlatformAPI::VideoSizeChangedCB& video_size_changed_cb,
  ...
  return base::MakeRefCounted<MojoMediaPlatformAPI>(
      media_task_runner, is_video, app_id, video_size_changed_cb,
...
void MojoMediaPlatformAPI::OnVideoSizeChanged(const gfx::Size& coded_size,
                                              const gfx::Size& natural_size) {
  if (!video_size_changed_cb_.is_null())
    video_size_changed_cb_.Run(coded_size, natural_size);
}
