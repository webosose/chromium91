...
SystemServiceBridgeDelegateWebOS::SystemServiceBridgeDelegateWebOS(
  ...
  luna::Client::Params luna_params;
  if (params.name.empty()) {
    if (params.suffix == -1) {
      luna_params.name =
          luna::GetServiceNameWithRandSuffix(params.appid.c_str(), "-");
    } else {
      luna_params.name =
          luna::GetServiceName(params.appid.c_str(), params.suffix, "-");
    }
  } else {
    luna_params.name = std::move(params.name);
  }
