...
void UMediaClientImpl::DispatchSourceInfo(
  ...
  if (sourceInfo.numPrograms > 0 && sourceInfo.programInfo.size()) {
    ...
    if (duration_ != duration) {
      duration_ = duration;
      if (event_listener_) {
        NEVA_VLOGTF(1) << "duration change - " << duration_
    ...
    if (has_video_ && sourceInfo.programInfo[0].videoTrackInfo.size() &&
      ...
      gfx::Size coded_video_size =
          gfx::Size(sourceInfo.programInfo[0].videoTrackInfo[0].width,
                    sourceInfo.programInfo[0].videoTrackInfo[0].height);
      gfx::Size natural_video_size =
          media::GetNaturalSize(coded_video_size, pixel_aspect_ratio_.width(),
                                pixel_aspect_ratio_.height());

      if (coded_video_size_ != coded_video_size ||
          natural_video_size_ != natural_video_size) {
        coded_video_size_ = coded_video_size;
        natural_video_size_ = natural_video_size;
        if (event_listener_)
          event_listener_->OnVideoSizeChanged();
      }
...
void UMediaClientImpl::DispatchVideoInfo(
  ...
  has_video_ = true;
  gfx::Size coded_video_size = gfx::Size(video_info.width, video_info.height);
