...
void VideoFrameProviderImpl::CreateStreamTextureProxyIfNeeded() {
  NEVA_DVLOGTF(1) << "stream_id_:" << stream_id_
...
void VideoFrameProviderImpl::UpdateVideoFrame() {
  ...
  if (frame_type_ == FrameType::kTexture && !stream_id_)
    CreateStreamTexture();
  ...
  switch (frame_type) {
    case FrameType::kTexture: {
      ...
      if (texture_mailbox_sync_token.namespace_id() ==
        ...
        gl->VerifySyncTokensCHROMIUM(sync_tokens, base::size(sync_tokens));
      }
...
void VideoFrameProviderImpl::SetFrameType(FrameType type) {
  ...
  frame_type_ = type;
  if (frame_type_ != FrameType::kTexture)
    UpdateVideoFrame();
}
