...
AudioDecoderMock::AudioDecoderMock(
...
void AudioDecoderMock::Initialize(
...
bool AudioDecoderMock::FeedForPlatformMediaAudioDecoder(
    const scoped_refptr<DecoderBuffer>& buffer) {
  DCHECK(task_runner_->BelongsToCurrentThread());
  DCHECK(buffer);
  DCHECK(media_platform_api_);

  return media_platform_api_->Feed(buffer, FeedType::kAudio);
}
