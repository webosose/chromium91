...
scoped_refptr<VideoFrame> ExternalRenderer::Render(
...
void ExternalRenderer::AudioBufferReady(AudioDecoderStream::ReadResult result) {
  DCHECK(task_runner_->BelongsToCurrentThread());

  if (result.code() == StatusCode::kAborted ||
      render_state_ == RenderState::kFlushing ||
...
void ExternalRenderer::VideoFrameReady(VideoDecoderStream::ReadResult result) {
  DCHECK(task_runner_->BelongsToCurrentThread());
...
void ExternalRenderer::AttemptRead() {
  ...
  switch (render_state_) {
    case RenderState::kPlaying:
      // Check |playback_rate_| to prevent continuous consuming buffers on
      // paused video.
      // But we should send |BUFFERING_HAVE_ENOUGH| to |client_| even if
      // |playback_rate_| is 0.
      // Because applicaiton can wait |BUFFERING_HAVE_ENOUGH| when
      // |playback_rate_| is 0.
      // (And then normally, application change the |playback_rate| as 1.)
      // So if |buffering_state_| is not |BUFFERING_HAVE_ENOUGH|, we need to
      // feed the data until |buffering_state_| is |BUFFERING_HAVE_ENOUGH|.
      if (playback_rate_ <= 0 && buffering_state_ == BUFFERING_HAVE_ENOUGH)
        return;
