...
WebOSForeignVideoWindow::WebOSForeignVideoWindow(
  ...
  VLOG(1) << " manager= " << manager << " / compositor= " << compositor
...
void WebOSForeignVideoWindow::SetNativeWindowId(
    const std::string& native_window_id) {
  native_window_id_ = native_window_id;
}

wl_webos_exported* WebOSForeignVideoWindow::GetWebOSExported() {
  return webos_exported_;
}

std::string WebOSForeignVideoWindow::GetNativeWindowId() {
  return native_window_id_;
}

void WebOSForeignVideoWindow::UpdateGeometry(
...
void WebOSForeignVideoWindow::SetProperty(const std::string& name,
                                          const std::string& value) {
  wl_webos_exported_set_property(webos_exported_, name.c_str(), value.c_str());
  manager_->Flush();
}

void WebOSForeignVideoWindow::SetVisibility(bool visibility) {
  if (visibility)
