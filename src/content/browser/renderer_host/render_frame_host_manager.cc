...
bool RenderFrameHostManager::IsRendererTransferNeededForNavigation(
  ...
  if (!rfh->GetSiteInstance()->IsSuitableForUrlInfo(dest_url_info))
    return true;
  ...
  if (ShouldSwapBrowsingInstancesForDynamicIsolation(
          rfh, dest_url_info, cross_origin_isolated_info))
    return true;
  ...
  if (ShouldProactivelySwapBrowsingInstance(
          dest_url_info, cross_origin_isolated_info, is_reload,
  ...
  auto current_site_info =
      SiteInfo::Create(rfh->GetSiteInstance()->GetIsolationContext(),
                       dest_url_info, cross_origin_isolated_info);
  if (rfh->GetSiteInstance()->RequiresDedicatedProcess() ||
      current_site_info.RequiresDedicatedProcess(
          rfh->GetSiteInstance()->GetIsolationContext())) {
    return true;
  }
...
RenderFrameHostManager::GetSiteInstanceForNavigationRequest(
  ...
  if (frame_tree_node_->IsMainFrame()) {
    ...
    no_renderer_swap_allowed |= !CanSubframeSwapProcess(
        ...
        request->dest_site_instance(), request);
  }
...
bool RenderFrameHostManager::CanSubframeSwapProcess(
  ...
  if (!IsRendererTransferNeededForNavigation(
          render_frame_host_.get(),
          UrlInfo(resolved_url,
