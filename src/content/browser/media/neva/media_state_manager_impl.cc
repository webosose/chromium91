...
void MediaStateManagerImpl::PermitMediaActivation(
  ...
  MediaWebContentsObserver* media_observer =
      web_contents_impl(web_contents)->media_web_contents_observer();
  if (!media_observer)
    return;

  media_observer
      ->GetMediaPlayerRemote(MediaPlayerId(
          render_frame_host->GetGlobalFrameRoutingId(), player_id))
      ->RequestActivation();
}
...
WebContentsImpl* MediaStateManagerImpl::web_contents_impl(
