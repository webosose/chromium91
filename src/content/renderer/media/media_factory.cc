...
params_neva->set_application_id(
  ...
  // TODO(neva, sync-to-87): Check |additional_contents_scale| is valid.
  // TODO(neva, sync-to-91): Ditto. Also deprecate the variable if possible.
  blink::WebFrameWidget* web_frame_widget =
      render_frame_->GetLocalRootWebFrameWidget();
  if (web_frame_widget) {
    const gfx::PointF scale =
        web_frame_widget->GetOriginalScreenInfo().additional_contents_scale;
    params_neva->set_additional_contents_scale(scale);
  }
    ...
    factory_selector->AddFactory(
        ...
        std::make_unique<media::NevaMediaPlayerRendererFactory>(
            ...
            base::Bind(&RenderThreadImpl::GetGpuFactories,
                       base::Unretained(render_thread))));
    factory_selector->SetBaseFactoryType(FactoryType::kNevaMediaPlayer);
  }
#endif
