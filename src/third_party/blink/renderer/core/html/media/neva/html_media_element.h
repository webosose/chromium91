...
class HTMLMediaElementExtendingWebMediaPlayerClient
 ...
 public:
  ...
  gfx::Rect ScreenRect() override;
  WebMediaPlayer::RenderMode RenderMode() const override;
  gfx::Rect WebWidgetViewRect() override;

  // Neva audio focus extension
  void OnAudioFocusChanged() override;

  void DidMediaCreated(bool will_use_media_resource) override;
  void DidMediaActivated() override;
  void DidMediaActivationNeeded() override;
  void DidMediaSuspended() override;
};
...
HTMLMediaElementExtendingWebMediaPlayerClient<original_t>::ScreenRect() {
  ...
  // incorrect value.
  return frame->GetPage()->GetChromeClient().GetScreenInfo(*frame).rect;
}
...
template <typename original_t>
void HTMLMediaElementExtendingWebMediaPlayerClient<original_t>::DidMediaCreated(
    bool will_use_media_resource) {
  original_t* self(static_cast<original_t*>(this));

  for (auto& observer : self->media_player_observer_remote_set_->Value())
    observer->OnMediaCreated(will_use_media_resource);
}

template <typename original_t>
void HTMLMediaElementExtendingWebMediaPlayerClient<
    original_t>::DidMediaActivated() {
  original_t* self(static_cast<original_t*>(this));

  for (auto& observer : self->media_player_observer_remote_set_->Value())
    observer->OnMediaActivated();
}

template <typename original_t>
void HTMLMediaElementExtendingWebMediaPlayerClient<
    original_t>::DidMediaActivationNeeded() {
  original_t* self(static_cast<original_t*>(this));

  for (auto& observer : self->media_player_observer_remote_set_->Value())
    observer->OnMediaActivationNeeded();
}

template <typename original_t>
void HTMLMediaElementExtendingWebMediaPlayerClient<
    original_t>::DidMediaSuspended() {
  original_t* self(static_cast<original_t*>(this));

  for (auto& observer : self->media_player_observer_remote_set_->Value())
    observer->OnMediaSuspended();
}
