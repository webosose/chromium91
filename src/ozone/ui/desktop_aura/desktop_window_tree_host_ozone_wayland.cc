  ...
  if (native_shape) {
    ...
    if (!transform.IsIdentity() && !native_region.isEmpty()) {
      ...
      if (native_region.getBoundaryPath(&path_in_dip)) {
        path_in_dip.transform(static_cast<SkMatrix>(transform.matrix()), &window_mask);
      }
    }
...
void DesktopWindowTreeHostOzone::SetZOrderLevel(ui::ZOrderLevel order) {
  z_order_level_ = order;
}

ui::ZOrderLevel DesktopWindowTreeHostOzone::GetZOrderLevel() const {
...
bool DesktopWindowTreeHostOzone::SetWindowTitle(const std::u16string& title) {
...
DesktopWindowTreeHostOzone::CreateNonClientFrameView() {
  NOTIMPLEMENTED();
  return NULL;
}
...
void DesktopWindowTreeHostOzone::OnBoundsChanged(
  ...
  OnHostResizedInPixels(change.bounds.size());
  ResetWindowRegion();
}
...
void DesktopWindowTreeHostOzone::OnWillDestroyAcceleratedWidget() {}
...
void DesktopWindowTreeHostOzone::OnMouseEnter() {
  auto* cursor_client = aura::client::GetCursorClient(window());
  if (cursor_client) {
    auto display =
...
void DesktopWindowTreeHostOzone::InitOzoneWindow(
  ...
  if (bounds.width() == 0)
    bounds.set_width(100);
  if (bounds.height() == 0)
    bounds.set_height(100);
  ...
  ui::neva::PlatformWindowType type =
      ui::neva::PlatformWindowType::kUnknown;
  switch (params.type) {
    case Widget::InitParams::TYPE_TOOLTIP: {
      type = ui::neva::PlatformWindowType::kTooltip;
      break;
    }
    case Widget::InitParams::TYPE_POPUP: {
      type = ui::neva::PlatformWindowType::kPopup;
      break;
    }
    case Widget::InitParams::TYPE_MENU: {
      type = ui::neva::PlatformWindowType::kMenu;
      break;
    }
    case Widget::InitParams::TYPE_BUBBLE: {
      type = ui::neva::PlatformWindowType::kBubble;
      break;
    }
    case Widget::InitParams::TYPE_WINDOW: {
      type = ui::neva::PlatformWindowType::kWindow;
      break;
    }
    case Widget::InitParams::TYPE_WINDOW_FRAMELESS: {
      type = ui::neva::PlatformWindowType::kWindowFrameless;
      break;
    }
    default:
      break;
  ...
  z_order_level_ = params.EffectiveZOrderLevel();
}
