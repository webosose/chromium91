...
auto& kMimeTypeText = ui::kMimeTypeText;
auto& kMimeTypeURIList = ui::kMimeTypeURIList;
...
void AddStringToOSExchangeData(ui::OSExchangeData* os_exchange_data,
  ...
  std::u16string string16 = base::UTF8ToUTF16(data);
...
void DesktopDragDropClientWayland::AddObserver(aura::client::DragDropClientObserver* observer) {
}

void DesktopDragDropClientWayland::RemoveObserver(aura::client::DragDropClientObserver* observer) {
}
...
void DesktopDragDropClientWayland::OnDragMotion(float x,
  ...
  if (event) {
    bool shouldAcceptDrag =
        delegate_->OnDragUpdated(*event).drag_operation ==
...
void DesktopDragDropClientWayland::OnDragDrop() {
  ...
  if (delegate_) {
    ...
    delegate_->OnPerformDrop(*event, std::move(data));
...
void DesktopDragDropClientWayland::OnDragDataCollected(
  ...
  if (event) {
    ...
    if (delayed_drop_location_) {
      ...
      auto data = std::make_unique<ui::OSExchangeData>();
      delegate_->OnPerformDrop(*delayedEvent.get(), std::move(data));
      DragDropSessionCompleted();
...
DesktopDragDropClientWayland::CreateDropTargetEvent(
  ...
  return std::unique_ptr<ui::DropTargetEvent>(new ui::DropTargetEvent(
      ...
      gfx::PointF(target_location.x(), target_location.y()),
      gfx::PointF(root_location.x(), root_location.y()), drag_operations));
}
