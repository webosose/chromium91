...
void CreateBitmapFromPng(
  ...
  if (bitmap == nullptr) {
    LOG(ERROR) << "Unable to load bitmap from image path = " << path;
    return;
  }
...
void OzoneWaylandWindow::SetTitle(const std::u16string& title) {
  title_ = title;
  if (!sender_->IsConnected())
    return;
...
gfx::Rect OzoneWaylandWindow::GetBounds() const {
  return bounds_;
}
...
void OzoneWaylandWindow::SetCustomCursorFromBitmap(
  ...
  // FIXME(neva): Need to add convertion from neva_app_runtime::CustomCursorType
  // to mojom::CursorType (ui/base/cursor/mojom/cursor_type.mojom)
  ...
  PlatformCursor cursor = CursorFactory::GetInstance()->CreateImageCursor(
      mojom_cursor_type, *cursor_image, gfx::Point(hotspot_x, hotspot_y));
