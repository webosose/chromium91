...
#ifndef OZONE_WAYLAND_GROUP_WEBOS_SURFACE_GROUP_H
#define OZONE_WAYLAND_GROUP_WEBOS_SURFACE_GROUP_H
...
class OZONE_WAYLAND_EXPORT WebOSSurfaceGroup : public wl_webos_surface_group {
 public:
  enum ZHint {
    ZHintBelow = 0,
    ZHintAbove,
  ...
  WebOSSurfaceGroup(const WebOSSurfaceGroup&) = delete;
  WebOSSurfaceGroup& operator=(const WebOSSurfaceGroup&) = delete;
  ~WebOSSurfaceGroup();

  void SetAllowAnonymousLayers(bool allow);
  void AttachAnonymousSurface(ZHint hint = ZHint::ZHintAbove);

  WebOSSurfaceGroupLayer* CreateNamedLayer(const std::string& name,
                                           int z_order);
...
#endif  // OZONE_WAYLAND_GROUP_WEBOS_SURFACE_GROUP_H
